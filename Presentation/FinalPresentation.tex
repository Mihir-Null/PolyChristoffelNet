...
\begin{itemize}
  \item Physics-style learning often wants \textbf{structure}, not just prediction:
  \begin{itemize}
    \item invariances, conservation laws, interpretable coefficients
    \item extrapolation beyond the training distribution
  \end{itemize}
  \item Many neural dynamics models use generic nonlinearities with limited interpretability.
...
\begin{frame}{Geometric intuition: ``forces'' as coordinate effects}
\begin{itemize}
...
\begin{frame}{Background: metric, connection, geodesics (minimal)}
\begin{itemize}
...
\begin{frame}{Integrator: ResNet-style rollout (discrete geodesic flow)}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figs/AE0PolarFig1.png}
    \caption{\scriptsize Learned vs. GT Christoffel components on $(r,\theta)$ grid.}
    \includegraphics[width=\linewidth]{figs/AE1PolarChrist.png}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figs/metric.png}
    \caption{\scriptsize Learned Metric}
    \includegraphics[width=\linewidth]{figs/truemetric.png}
    \caption{\scriptsize True Metric}
    \caption{\scriptsize Reconstructed $g$ components and loop error heatmap.}
  \end{figure}
\end{columns}
\end{frame}
...
\begin{frame}{AutoEncoded metrics)}
  \[
  While the AutoEncoded metrics are a little harder to interpret due to nonlinear coordinate transforms, we can still see what metric values we get by reinterpreting the embeddings under the ground truth metric
  \]
  \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figs/ae_metric.png}
    \caption{\scriptsize Learned Metric}
    \includegraphics[width=\linewidth]{figs/truemetric.png}
    \caption{\scriptsize True Metric}
    \caption{\scriptsize Reconstructed $g$ components and loop error heatmap.}
...
\begin{frame}{Research directions (compact)}
\begin{itemize}
    \item Enforce a Levi-Civita connection instead of just torsionlessness to ensure small closed loops return to the same point
    \item Enforce a notion of "distance" via states through known equivalencies
